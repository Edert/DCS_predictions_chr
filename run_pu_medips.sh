...
if [ "$files" = "0" ]; then
  ...
  echo "shift=0" >> $SCRIPT
  echo "ws=100" >> $SCRIPT
  echo "chr.select=c('chr19','chr11','chr8','chrX','chr1')" >> $SCRIPT
  ...
  echo "resultTable = MEDIPS.meth(MSet1 = S1, MSet2 = S2, CSet = CS, ISet1 = S1_Input, ISet2 = S2_Input, chr = chr.select, p.adj='BH', diff.method='edgeR', CNV=FALSE, MeDIP=FALSE, diffnorm='tmm')" >> $SCRIPT
  ...
  echo "S2_Input = MEDIPS.createSet(file = '$9', BSgenome = BSgenome,extend = extend, shift = shift, uniq = uniq, window_size = ws, chr.select = chr.select)" >> $SCRIPT
  echo "CS = MEDIPS.couplingVector(pattern = 'CG', refObj = S1[[1]])" >> $SCRIPT
  echo "resultTable = MEDIPS.meth(MSet1 = S1, MSet2 = S2, CSet = CS, ISet1 = S1_Input, ISet2 = S2_Input, chr = chr.select, p.adj='BH', diff.method='edgeR', CNV=FALSE, MeDIP=FALSE, diffnorm='quantile')" >> $SCRIPT
  echo "sig = MEDIPS.selectSig(results=resultTable, p.value=0.05, adj=TRUE, ratio=NULL, bg.counts=NULL, CNV=FALSE)" >> $SCRIPT
  echo "options(scipen = 999)" >> $SCRIPT
  echo "write.table(file='results.txt', sig, quote = FALSE, sep = \"\t\",row.names = FALSE, col.names = FALSE)" >> $SCRIPT
  ...
  echo "S2_Input = MEDIPS.createSet(file = '$9', BSgenome = BSgenome,extend = extend, shift = shift, uniq = uniq, window_size = ws, chr.select = chr.select)" >> $SCRIPT
  echo "CS = MEDIPS.couplingVector(pattern = 'CG', refObj = S1[[1]])" >> $SCRIPT
  echo "resultTable = MEDIPS.meth(MSet1 = S1, MSet2 = S2, CSet = CS, ISet1 = S1_Input, ISet2 = S2_Input, chr = chr.select, p.adj='BH', diff.method='edgeR', CNV=FALSE, MeDIP=FALSE, diffnorm='none')" >> $SCRIPT
  echo "sig = MEDIPS.selectSig(results=resultTable, p.value=0.05, adj=TRUE, ratio=NULL, bg.counts=NULL, CNV=FALSE)" >> $SCRIPT
  echo "options(scipen = 999)" >> $SCRIPT
  echo "write.table(file='results.txt', sig, quote = FALSE, sep = \"\t\",row.names = FALSE, col.names = FALSE)" >> $SCRIPT
