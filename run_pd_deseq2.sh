...
if [ "$files" = "0" ]; then
  ...
  #for PCALLER in "../../../results_peaks/sicer"; do
    ...
    for PMODE in $PCALLER/$SET/*; do
      ...
      echo "group <- factor(c('S1','S1','S2','S2'))" >> $SCRIPT
      echo "cnts <- cov[,4:7]" >> $SCRIPT
      echo "dds <- DESeqDataSetFromMatrix(countData = cnts,colData = DataFrame(group),design = ~ group)" >> $SCRIPT
      echo "dds <- DESeq(dds)" >> $SCRIPT
      echo "res <- results(dds, contrast = c('group','S2','S1'))" >> $SCRIPT
      echo "results <- cbind(as.character(cov\$V1),cov\$V2,cov\$V3,res\$padj,res\$log2FoldChange)" >> $SCRIPT
      echo "options(scipen = 999)" >> $SCRIPT
      echo "write.table(file='results.csv', results, quote = FALSE, row.names=F, col.names=F, sep ='\t')" >> $SCRIPT
